...
标准的微调框架（如 _LLaMA-Factory_）主要基于 `AutoModelForCausalLM` 设计，无法原生支持判别式的序列分类任务。
...
├── models/
│   └── Gemma-2-9B/
│   └── Gemma2-9B-cv93/         # 自动保存的 LoRA 各步数 Checkpoints 及权重(上传到huggingface 后会更新网址)
├── output/
...
![training_loss_curves](D:\.FFF\个人\myProject\Charting Student Math Misunderstandings\output\training_loss_curves.png)
...
_提示：如果您的 bitsandbytes 版本过低导致无法启动，`train_seqcls.py` 会在最顶端自动拦截并为您静默升级至符合要求的兼容版本。_
...
_脚本内含路径自动 fallback 逻辑，可在您的本地环境目录 `models/` 与 Autodl 云端目录 `/root/autodl-tmp/` 之间无缝切换。_
...
本项目专门针对 _Kaggle: Charting Student Math Misunderstandings_ 竞赛进行算法深度调优，基于 Hugging Face PEFT 与 PyTorch 框架构建。
感谢项目原作者的启发https://www.kaggle.com/code/kishanvavdara/ensemble-gemma-qwen-deepseek